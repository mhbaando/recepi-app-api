$(document).ready(() => {
  ...
  // also check file uppload size
  $('#doc').change(function () {
    const file = this.files[0];
    if (file) {
      if (file.size > 2097152) {
        Swal.fire('Error', 'File Exceedes Max size of 2MB', 'error')
        $('input[type="file"]').val('')
        return false;
      }
  ...
  $('#transfer_form').on('submit', function (e) {
    ...
    if (reason.val().trim().length <= 0) {
      return Swal.fire("Error", "Fill Transfere Reason with transfer detail", "error");
    }

$(document).ready(function () {
  ...
  $("#state_name").on("change", function () {
    ...
    if (state === 'Select State' || !state) {
      $("#place_issuess").empty();
      $("#place_issuess").append(`<option value="">Select A Place of Issue</option> `)
      $("#place_issuess").prop('disabled', true)
    } else {
      fetch('/customer/state_r_op/' + state, {
        ...
        if ($(this).val()) {
          $("#place_issuess").prop('disabled', false)
          $("#place_issuess").empty();
          $("#place_issuess").append(`<option value="${data.s_states}">${data.s_names}</option> `)
        } else {
          $("#place_issuess option:eq(1)").prop("selected", true);
          $("#place_issuess").prop('disabled', true)
        }
      }).catch(err => {
...
$("#submit").on("click", function () {
  ...
  let place_issue = $("#place_issuess").val();

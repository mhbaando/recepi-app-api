$(document).ready(function () {
  ...
  function RefreshPage() {
    ...
    if (search == "") {
      window.location.replace(
        `/users/employee_list?DataNumber=${entries}&page=${page}`
      );
    } else {
      window.location.replace(
      
          `/users/employee_list?DataNumber=${entries}&SearchQuery=${search}&page=${page}`
      );
    }
  }
  ...
  $("#users_table tbody").on(
    "click",
    "#manage_employee_user_status",
    function (e) {
      e.preventDefault();
      ...
      Swal.fire({
        title: "Are you sure?",
        text: "To change the user status",
        icon: "warning",
        showCancelButton: !0,
        ...
        if (e.value) {
          $.ajax({
            method: "POST",
            url: "/employees/manage_employee/UsersStatus",
            headers: { "X-CSRFToken": csrftoken },
            processData: false,
            contentType: false,
            data: formData,
            ...
            success: function (response) {
              if (!response.isError) {
                if (action_Type == true) {
                  inputEvent.prop("checked", true);
                } else {
                  inputEvent.prop("checked", false);
                }
                Swal.fire({
